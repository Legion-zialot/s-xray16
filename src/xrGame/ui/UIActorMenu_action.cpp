...
{
    //Alundaio: Здесь мы экспортируем действие перетаскивания одного предмета инвентаря на другой!
    luabind::functor<bool> funct1;
    if (GEnv.ScriptEngine->functor("actor_menu_inventory.CUIActorMenu_OnItemDropped", funct1))
    {
        //Если в списке только 1 элемент, получить его, в противном случае попытаться получить элемент в текущей позиции перетаскивания.
        ...
        //Обратный вызов обрабатывает сброс предмета на предмет, в других случаях (перемещение, сброс из инвентаря) мы его не запускаем.
        if (_iitem == nullptr)
            return true;
