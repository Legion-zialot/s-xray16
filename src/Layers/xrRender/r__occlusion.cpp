...
u32 R_occlusion::occq_begin(u32& ID)
{
    ...
    //	Игорь: предотвращает сбой релиза, если мы отправим слишком много запросов
    // Вадим: изменено условие вывода предупреждения по pool.size() не нулевого значения
    if (pool.empty())
    {
        if ((Device.dwFrame % 40) == 0)
            if(pool.size() > 0) Msg(" RENDER [Warning]: Too many occlusion queries were issued(>%zu)!!!", pool.size());
        ID = iInvalidHandle;
        return 0;
    }
...
R_occlusion::occq_result R_occlusion::occq_get(u32& ID)
{
    ...
    // вставить в пул (сортировка по убыванию)
    _Q& Q = used[ID];
    if (pool.empty())
        pool.push_back(Q);
    else
    ...
    // извлечь из использования и дать усадку по мере необходимости
    used[ID].Q = 0;
    fids.push_back(ID);
    ID = 0;
    return fragments;
